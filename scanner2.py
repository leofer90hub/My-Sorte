...
FICHEIRO_DE_SAIDA = "HIT_V2.txt"
FICHEIRO_CHECKPOINT = "checkpoint_v2.json"
    ...
    while True:
        ...
        for i in range(CHAVES_POR_PAGINA):
            ...
            try:
                ...
                # --- ADICIONADO FORMATO NÃO COMPRIMIDO ---
                h_comp = hashlib.new('ripemd160', hashlib.sha256(pk.format(True)).digest()).digest()
                h_uncomp = hashlib.new('ripemd160', hashlib.sha256(pk.format(False)).digest()).digest()
                
                # Verifica ambos no SET (Comprimido e Não Comprimido)
                for h_atual in [h_comp, h_uncomp]:
                    if h_atual in alvos_set:
                        tipo = "COMP" if h_atual == h_comp else "UNCOMP"
                        info = f"HIT! [{tipo}] URL: {url_site}\nHEX: {priv.hex()}"
                        with lock:
                            if h_atual not in emails_enviados:
                                enviar_email("BITCOIN FOUND!", info)
                                emails_enviados.append(h_atual)
...
if __name__ == "__main__":
    ...
    # Mantido rigorosamente 1 worker conforme pedido
    for i in range(1):
